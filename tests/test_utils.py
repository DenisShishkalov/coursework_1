...
from src.utils import (filter_transactions_by_date, get_info_cards, reading_to_file, get_exchange_rates,
                       get_stocks_cost, get_top_5_transaction, greeting)
...
def test_get_data_from_xlsx():
    ...
    with patch("pandas.read_excel", return_value=df):
        result = reading_to_file(r"../data/operations_excel.xlsx")
...
def test_get_cards_data_empty():
    transactions = []
    expected_result = []
    assert get_info_cards(transactions) == expected_result
...
def test_get_top_5_transactions_empty():
    transactions = []
    expected_result = []
    assert get_top_5_transaction(transactions) == expected_result
